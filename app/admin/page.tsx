...
export default async function AdminDashboardOverview() {
  ...
  try {
    ...
    pendingComplaints = await prisma.complaint.count({ where: { status: "OPEN" } }) || await prisma.complaint.count({ where: { status: "PENDING" } });
    unreadInquiries = await prisma.inquiry.count({ where: { status: "UNREAD" } });

    // Calculate Estimated Total Revenue from Approved & Completed Bookings
    const revenueBookings = await prisma.booking.findMany({
      where: { status: { in: ["APPROVED", "COMPLETED"] } },
      select: { totalPrice: true }
    });
    estimatedRevenue = revenueBookings.reduce((sum, b) => sum + (b.totalPrice || 0), 0);
    ...
    // Upcoming Check-ins for reception
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5 mb-10">
        <StatCard title="Total Revenue" value={`₦${estimatedRevenue.toLocaleString()}`} description="From approved bookings" />
        ...
        <StatCard title="Pending Tickets" value={pendingComplaints} description="Reported issues" alert={pendingComplaints > 0} />
        <StatCard title="Unread Msgs" value={unreadInquiries} description="New contact msgs" alert={unreadInquiries > 0} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="glass p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="font-display text-xl text-white">Recent Inquiries</h2>
                <Link href="/admin/inquiries" className="text-sm text-[#ff7a1a] hover:underline">View all</Link>
              </div>
              <div className="flex flex-col gap-4">
                {recentInquiries.length > 0 ? (
                  recentInquiries.map((inq) => (
                    <div key={inq.id} className={`p-4 rounded-xl border ${inq.status === 'UNREAD' ? 'border-[#ff7a1a]/30 bg-[rgba(255,122,26,0.05)]' : 'border-white/5 bg-white/5'}`}>
                      <div className="flex justify-between mb-1">
                        <strong className="text-white text-sm truncate">{inq.name}</strong>
                        {inq.status === 'UNREAD' && <span className="text-[10px] bg-[#ff7a1a] text-[#111] font-bold px-1.5 rounded">NEW</span>}
                      </div>
                      <div className="text-[#b1b1ba] text-xs mb-2">{inq.subject}</div>
                      <p className="text-gray-400 text-xs line-clamp-2 italic">"{inq.message}"</p>
                    </div>
                  ))
                ) : (
                  <div className="py-8 text-center text-gray-500 text-sm italic">No inquiries received yet.</div>
                )}
              </div>
            </div>
            ...
            <div className="glass p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="font-display text-xl text-white">Expected Check-Ins</h2>
                <Link href="/admin/bookings" className="text-sm text-[#ff7a1a] hover:underline">View all</Link>
              </div>
              <div className="flex flex-col gap-4">
                {expectedCheckins.length > 0 ? (
                  expectedCheckins.map((booking) => {
                    const isToday = new Date(booking.checkInDate).toDateString() === new Date().toDateString();
                    return (
                      <div key={booking.id} className={`p-4 rounded-xl border flex flex-col ${isToday ? 'border-green-500/30 bg-green-500/5' : 'border-blue-500/20 bg-blue-500/5'}`}>
                        <div className="flex justify-between mb-1">
                          <strong className="text-white text-sm truncate">{booking.residentName}</strong>
                          {isToday && <span className="text-[10px] bg-green-500 text-green-950 font-bold px-1.5 rounded">ARRIVING TODAY</span>}
                        </div>
                        <div className="text-xs text-gray-400 mb-2">
                          {booking.listing?.title} • {booking.durationCount} {booking.duration}
                        </div>
                        <div className="mt-auto pt-2 justify-between flex items-center border-t border-white/5 w-full text-xs">
                          <span className="text-gray-300 font-medium">Date: {new Date(booking.checkInDate).toLocaleDateString()}</span>
                          <span className="text-gray-500">{booking.residentPhone}</span>
                        </div>
                      </div>
                    )
                  })
                ) : (
                  <div className="py-8 text-center text-gray-500 text-sm italic border border-dashed border-[rgba(255,255,255,0.1)] rounded-xl">No upcoming check-ins scheduled.</div>
                )}
              </div>
            </div>
          </div>
...
function StatCard({ title, value, description, alert = false }: { title: string, value: string | number, description: string, alert?: boolean }) {
  return (
